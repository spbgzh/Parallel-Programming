...
#pragma omp parallel for default(none) shared(N, M1) schedule(static, 4)
    for (int j = 0; j < N; j++)
    {
      M1[j] = exp(sqrt(M1[j]));
    }
...
#pragma omp parallel for default(none) shared(N, M2, M2CP) schedule(static, 4)
    for (int j = 0; j < N / 2; j++)
    {
...
#pragma omp parallel for default(none) shared(N, M1, M2) schedule(static, 4)
    /* Этап Merge */
    for (int j = 0; j < N / 2; j++) {
      M2[j] = M1[j] / M2[j];
    }
...
#pragma omp parallel for default(none) shared(N, M2, min) reduction(+:X) schedule(static, 4)
    for (int j = 0; j < N / 2; j++) {
      if ((int)(M2[j] / min) % 2 == 0) {
#pragma omp atomic
        X+= sin(M2[j]);
